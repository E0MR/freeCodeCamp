...
function getMaxProfit(prices, budget) {
  ...
  for (let i = 0; i < prices.length; i++) {
    let stocks = Math.floor(budget / prices[i]);
    let tempMaxProfit = 0;

    for (let j = i + 1; j < prices.length; j++) {
      let currentProfit = prices[j] * stocks;

      tempMaxProfit =
        currentProfit > tempMaxProfit ? currentProfit : tempMaxProfit;
    }

    let netProfit = tempMaxProfit > 0 ? (tempMaxProfit - (stocks * prices[i])) : 0;
